...
try:
    ...
    # actual_text = driver.find_element(By.XPATH, "//div[@class='feed-com-text-inner']/div/div/a[@class='blog-p-user-name'][contains(text(), 'Поладько')]").text
    print("actual_text - ", actual_text)
    needed_text = user_1
    # # (//div[@class="feed-post-text"])[1]
    assert actual_text == needed_text
    print("Проверка по ФИО - Ок")
    """проверка по урл"""
